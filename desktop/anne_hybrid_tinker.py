...
class HybridTinker(tk.Tk):
    ...
    def _build(self) -> None:
        ...
        ttk.Label(header, text="QWEN ÖĞRETMEN → ANNE ANALİZ → ÖRÜNTÜ ÖĞRENME → SONRAKİ QWEN CEVABI").grid(row=0, column=0, sticky="w", padx=8, pady=6)
        ...
        ttk.Checkbutton(header, text="Qwen cevabından öğren", variable=self.learning).grid(row=3, column=0, sticky="w", padx=8, pady=5)

        info = ttk.LabelFrame(root, text="ANNE Veri Alanı")
        info.grid(row=1, column=0, sticky="ew", pady=(0, 8))
        ttk.Label(info, text=f"Kalıcı veri: {anne_data_root()}    •    Eğitim: {len(LESSONS)} ders").pack(anchor="w", padx=8, pady=6)
        ...
        ttk.Button(bottom, text="Gönder", command=self.send).grid(row=0, column=1, sticky="ns", padx=(7, 0))
    ...
    def _refresh(self) -> None:
        ...
        self.lesson.configure(text=f"DERS {lesson.id}: {lesson.title} — {lesson.practice}")
        self.status.configure(text=f"ANNE hazır • {stats['packets']} paket • {stats['style_patterns']} cevap örüntüsü • {stats['rules']} kural")
    ...
    def _analyze_response(response: str) -> dict[str, list[str]]:
        ...
        if any(word in text.lower() for word in ("ancak", "fakat", "öte yandan", "belirsiz", "kesin değil")):
    ...
    def _worker(self, text: str) -> None:
        try:
            ...
            learned = self.transfer.context(limit=18, topic=text)
            self.engine.cycle(text, memory=memory_context, knowledge=learned)
            state_json = str(self.engine.snapshot())
            system = (
                "Sen ANNE'nin öğretmen modelisin. ANNE kendi bilişsel durumunu oluşturur; sen güçlü bir cevap örneği ver. "
                "Cevabın açık, gerekçeli, örnekli ve belirsizliği dürüstçe belirten bir yapıda olsun. "
                f"Bugünkü ders hedefi: {lesson.objective} Pratik şablon: {lesson.practice}. "
                "Önceki ANNE öğrenmelerini körü körüne doğru kabul etme; gerektiğinde düzelt."
            )
            prompt = (
                f"ANNE COGNITIVE STATE:\n{state_json}\n\n"
                f"ÖĞRENİLMİŞ ÖRÜNTÜLER:\n{learned or '(henüz yok)'}\n\n"
                f"DERS:\n{lesson.title} — {lesson.objective}\n\n"
                f"SORU:\n{text}"
            )
            provider = EmbeddedAIProvider(n_ctx=2048, n_threads=2, max_tokens=380)
            ...
            if self.learning.get():
                ...
                self.engine.ogren(response, "Qwen cevabı analiz edildi; cevap örüntüleri ANNE bilgi deposuna kaydedildi.")
                self.memory.remember(text)
                self.memory.save(text, response, "Qwen cevabı ANNE tarafından analiz edildi ve örüntüler çıkarıldı.", state_json.count("confidence"))
        ...
        try:
            while True:
                ...
                if kind == "response":
                    ...
                    data = payload
                    self._append("ANNE ANALİZ", "Öğrendiği cevap örüntüleri:\n" + "\n".join([*("• " + x for x in data["style"]), *("• " + x for x in data["patterns"]), *("• " + x for x in data["rules"])]))
                elif kind == "status":
