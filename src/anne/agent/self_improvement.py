...
class SelfImprovementEngine:
    ...
    def validate_plan(cls, plan: SelfImprovementPlan) -> None:
        ...
        if plan.base.lower() in GitHubRepoTool.PROTECTED_BRANCHES:
            # The protected base is intentionally allowed for the PR target;
            # the writable branch is what the engine guards.
            pass
        if not plan.changes:
        ...
        for change in plan.changes:
            ...
            if action in {"update", "delete"} and not change.sha:
                raise ValueError(
                    f"Current file SHA is required for {action}: {change.path}"
                )
    ...
    def apply(
        ...
        *,
        ...
        """Apply the plan on a new feature branch and open a pull request.

        The plan must already be approved by the caller. No merge operation is
        performed here.
        """
        ...
        for change in plan.changes:
            ...
            if action == "create":
                ...
                assert change.content is not None and change.sha is not None
