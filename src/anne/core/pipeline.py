...
The pipeline is deterministic and does not require an LLM.
"""
...
class AnnePipeline:
    ...
    def __init__(
        ...
        self.hypothesis_engine = hypothesis_engine or HypothesisEngine(memory)
    ...
    def gor(self, state: CognitiveState, hypotheses: Sequence[Hypothesis]) -> CognitiveState:
        ...
        preserve_floor = max(0.05, min(0.30, winner.hypothesis.probability - 0.20))
    ...
    def run_with_hypotheses(
        ...
        """Run the full pipeline; generate local hypotheses when omitted."""
        ff = self.fail_fast(raw_input)
        if not ff.passed:
            cycle_id = (hypotheses[0].id if hypotheses else "cycle") if hypotheses else "cycle"
            self.memory.save_failure_trace(
                cycle_id=cycle_id,
                stage="FAIL_FAST",
                raw_input=raw_input,
                ...
                hypothesis_id=cycle_id if cycle_id != "cycle" else "",
                ethic_total=0.0,
            )
            return ff, None
        ...
        candidates = list(hypotheses) if hypotheses else self.hypothesis_engine.generate(
            raw_input,
            context={"input_type": state.input_type},
        )
        if not candidates:
            raise RuntimeError("HypothesisEngine produced no candidates")
        state = self.gor(state, candidates)
        ...
        selected = next(h for h in candidates if h.id == selected_id)
        ...
        for candidate in candidates:
            self.memory.save_hypothesis(candidate)
        return ff, state
    ...
    def run_with_fail_fast(
        ...
        """Backward-compatible entry point; hypothesis may now be omitted."""
        return self.run_with_hypotheses(raw_input, consciousnesses, None if hypothesis is None else [hypothesis])
