"""DecisionLoop — facade over FailFast, six-stage pipeline, and FTL."""
...
from anne.core.fractal_thinking_loop import FractalBudget, FractalCognitiveLoop, FractalResult
...
class DecisionLoop:
    """Mandatory entry path for ordinary and fractal cognitive cycles."""
    ...
    def run(
        ...
        """Execute one full six-stage decision cycle."""
        ...
        if not ff.passed:
            return DecisionResult(
                status="ABORTED", verdict="FAIL_FAST", action="HALT",
                output={"verdict": "FAIL_FAST", "action": "HALT", "reason": ff.reason, "rule_id": ff.rule_id},
    ...
    def run_fractal(
        ...
        *,
        ...
        """Execute the bounded multi-scale Fractal Cognitive Loop."""
        parties = list(parties) if parties else [Consciousness(id="user")]
        text_claim = claim if claim is not None else raw_input
        hyp = hypothesis or Hypothesis(
            id=f"h_{uuid4().hex[:12]}",
        ...
        return loop.run(raw_input, hyp, parties, task_mode=task_mode)
