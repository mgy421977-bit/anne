...
class FractalThinkingLoop:
    ...
    def _gap(text: str) -> bool:
        markers = ("unknown", "uncertain", "unclear", "missing", "unresolved", "contradiction",
                   "gap", "bilinmiyor", "belirsiz", "çelişki", "eksik", "kanıt yok")
        lowered = text.lower()
        return any(marker in lowered for marker in markers)

    def _record(self, node: FractalNode, task_mode: TaskMode) -> None:
        self.memory.save_scale_event(cycle_id=node.cycle_id, parent_cycle_id=node.parent_cycle_id,
            depth=node.depth, scale_role=node.scale_role, task_mode=task_mode.value,
            question=node.question, selected_claim=node.claim, status=node.status,
            stage_reached=node.stage_reached)

    def _failure(self, node: FractalNode, reason: str, task_mode: TaskMode) -> None:
        self.memory.save_failure_trace(cycle_id=node.cycle_id, stage=node.stage_reached,
            raw_input=node.question, reason=reason, meta_tag="fractal_loop",
            hypothesis_id=node.metadata.get("hypothesis_id", ""), depth=node.depth,
            parent_cycle_id=node.parent_cycle_id, task_mode=task_mode.value,
            scale_role=node.scale_role)

    def run(self, question: str, hypothesis: Hypothesis, *,
            parties: Sequence[Consciousness] | None = None,
            task_mode: TaskMode = TaskMode.GENERAL) -> FractalResult:
        people = list(parties) if parties else [Consciousness(id="user")]
        root_id = f"fc_{uuid4().hex[:12]}"
        root = FractalNode(root_id, None, 0, "frame", question, hypothesis.claim,
        ...
        while iterations < self.budget.max_iterations:
            ...
            node = root if iterations == 1 else FractalNode(
                f"fc_{uuid4().hex[:12]}", nodes[-1].cycle_id, min(nodes[-1].depth + 1, self.budget.max_depth),
                "branch", current_question, current.claim, metadata={"hypothesis_id": current.id})
            if node is not root:
                nodes.append(node); self._record(node, task_mode)

            ff, state = self.pipeline.run_with_fail_fast(current_question, people, current)
            if not ff.passed:
                node.status, node.stage_reached = "failed", "FAIL_FAST"; self._record(node, task_mode)
                self._failure(node, ff.reason, task_mode)
                return FractalResult("aborted", "", root_id, current.claim, 0.0, nodes, iterations, "fail_fast")
            assert state is not None
            node.stage_reached = "YAP"
            node.confidence = float(state.context_map.get("anla_score") or current.probability)
            last_answer = str(state.output.get("reasoning") or state.output.get("hypothesis") or current.claim)
            node.gap_detected = self._gap(current_question + " " + current.claim) or not state.logic_valid
            if state.logic_valid and not node.gap_detected:
                node.status = "integrated"; self._record(node, task_mode)
                return FractalResult("completed", last_answer, root_id, current.claim, node.confidence, nodes, iterations, "validated")

            if node.depth >= self.budget.max_depth:
                node.status, node.stage_reached = "stopped", "REFRAME"; self._record(node, task_mode)
                self._failure(node, "fractal_depth_budget_exhausted", task_mode)
                return FractalResult("bounded", last_answer, root_id, current.claim, node.confidence, nodes, iterations, "max_depth")
            ...
            if not selection.accepted or selection.candidate is None:
                ...
                return FractalResult("bounded", last_answer, root_id, current.claim, node.confidence, nodes, iterations, "abstain")

            selected = selection.candidate
            reframe = FractalNode(f"fc_{uuid4().hex[:12]}", node.cycle_id, node.depth + 1,
                "reframe", current_question, selected.claim, confidence=selection.score,
                stage_reached="REFRAME", metadata={"hypothesis_id": selected.id})
            nodes.append(reframe); self._record(reframe, task_mode)
            current = Hypothesis(id=selected.id, topic=selected.goal[:48], claim=selected.claim,
                                 probability=selected.probability, source="MITOS")
            current_question = selected.goal
            reframe.status, reframe.stage_reached = "integrated", "INTEGRATE"; self._record(reframe, task_mode)

        self._failure(nodes[-1], "fractal_iteration_budget_exhausted", task_mode)
        nodes[-1].status, nodes[-1].stage_reached = "stopped", "STOP"; self._record(nodes[-1], task_mode)
        return FractalResult("bounded", last_answer, root_id, current.claim, nodes[-1].confidence,
                             nodes, iterations, "max_iterations")
